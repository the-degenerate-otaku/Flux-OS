...
function setupArm() {
    ...
    var tentacles = new LimbSystem(node, 3, 8, critter);
    setInterval(function () {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
...
function setupLizard(size, legs, tail) {
    var s = size;
    if (reptileAnimationTimer !== null) {
        clearInterval(reptileAnimationTimer);
    }
    reptilePetConfig = { size: size, legs: legs, tail: tail };
    ...
    reptileAnimationTimer = setInterval(function () {
        ...
        targetY += (Input.mouse.y - targetY) * 0.18;
        critter.follow(targetX, targetY);
    }, 33);
}

function setReptilePetEnabled(enabled) {
    reptilePetEnabled = Boolean(enabled);
    localStorage.setItem("flux_reptile_pet", reptilePetEnabled ? "on" : "off");
    if (reptilePetEnabled) {
        setupLizard(reptilePetConfig.size, reptilePetConfig.legs, reptilePetConfig.tail);
    } else {
        clearInterval(reptileAnimationTimer);
        reptileAnimationTimer = null;
        ctx.clearRect(0, 0, canvas.width, canvas.height);
    }
}
...
var lizardSize = 6 / Math.sqrt(legNum);
var lizardTail = Math.floor(4 + Math.random() * legNum * 6);
if (reptilePetEnabled) {
    setupLizard(lizardSize, legNum, lizardTail);
} else {
    reptilePetConfig = { size: lizardSize, legs: legNum, tail: lizardTail };
}
